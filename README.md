...
- **neoscroll** — animated scrolling, so a half-page jump can be followed by eye rather than re-located. Durations are per key: 100ms for half-page (`<C-u>`/`<C-d>`/`<M-u>`/`<M-d>`), 160ms for full page, 80ms for `zz`/`zt`/`zb`, with `sine` easing — the curve reads as smoother than `linear` at the same duration. Below ~80ms the motion stops registering as motion and is just lag; above ~250ms it reads as waiting.
  - `<M-u>`/`<M-d>` call neoscroll's helpers directly rather than aliasing `<C-u>`/`<C-d>`: a `vim.keymap.set` alias is `noremap` by default, so it would run the built-in scroll and silently skip the animation.
...
- **Debugging (DAP)** — [nvim-dap](https://github.com/mfussenegger/nvim-dap) speaks the same wire protocol VS Code's debugger does, against the same `debugpy` adapter, so breakpoints, stepping and inspection behave identically. What VS Code adds on top is the UI, which [nvim-dap-ui](https://github.com/rcarriga/nvim-dap-ui) supplies (scopes, stacks, breakpoints, watches, repl); `nvim-dap-virtual-text` prints values inline beside the code as you step. Adapters are installed by [mason-nvim-dap](https://github.com/jay-babu/mason-nvim-dap.nvim) the way mason installs servers, and the python configurations come from [nvim-dap-python](https://github.com/mfussenegger/nvim-dap-python).
  ...
  - **Stepping** — `<leader>dl`/`<leader>dj`/`<leader>dk` step over/into/out, with the call stack drawn vertically: callees below, callers above, execution running left to right along the line. `<leader>dK`/`<leader>dJ` are the same directions with nothing executed — they move which frame you're *looking* at. `<F10>`/`<F11>`/`<S-F11>` do the same as VS Code's keys (`<F12>` is an unshifted step-out fallback). `<leader>dc` continue, `<leader>dC` run to cursor, `<leader>dR` re-run the last configuration, `<leader>dm` debug the test the cursor is inside.
